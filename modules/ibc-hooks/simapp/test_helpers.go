...
func setup(withGenesis bool, invCheckPeriod uint) (*App, GenesisState) {
	...
	app := NewSimApp(log.NewNopLogger(), db, nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, MakeEncodingConfig(), appOptions, bam.SetChainID("testchain"))
	if withGenesis {
...
func NewSimappWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *App {
	...
	app := NewSimApp(options.Logger, options.DB, nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, MakeEncodingConfig(), options.AppOpts, bam.SetChainID("testchain"))
...
func NewTestNetworkFixture() network.TestFixture {
	...
	app := NewSimApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, MakeEncodingConfig(), simtestutil.NewAppOptionsWithFlagHome(dir), bam.SetChainID("testchain"))

	appCtr := func(val network.ValidatorI) servertypes.Application {
		return NewSimApp(
			val.GetCtx().Logger, dbm.NewMemDB(), nil, true, map[int64]bool{}, DefaultNodeHome, simcli.FlagPeriodValue, MakeEncodingConfig(),
